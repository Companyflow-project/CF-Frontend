...
export const AddEmployeeAsContactModal: React.FC<AddEmployeeAsContactModalProps> = ({
  ...
  useEffect(() => {
    if (open && employee) {
      ...
      setSelectedTids(existingTids);
      setCustomAreas([]);
      setNewCustomArea('');
      setIsAddingCustomArea(false);
    }
  }, [open, employee, existingTids]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[680px] p-0 flex flex-col gap-0 rounded-[20px] overflow-hidden border border-[#e5efea] shadow-[0_20px_60px_rgba(14,51,38,0.15)] max-h-[90vh]">
        ...
        <div className="px-6 pb-5 space-y-2 pt-1 border-t border-[#f0f4f2]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
            {areasData.map((area) => {
              const isChecked = selectedTids.includes(area.tid);
