...
import { useTicketCreateOptions, useCreateTicket } from '../hooks';
...
export const AdminCreateTicketPage: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    const payload: CreateTicketPayload = {
      ...
      statusTid,
      responsibleUid,
      orientedUids: orientedUids.length > 0 ? orientedUids : undefined,
      listTids: listTids.length > 0 ? listTids : undefined,
      sendMail,
      published,
    };

    try {
      const { nid } = await createMutation.mutateAsync(payload);
      toast.success(t('createTicket.success', 'Ticket #{{nid}} created', { nid }));
      navigate(adminRoutes.tickets);
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      toast.error(t('createTicket.errors.saveFailed', 'Failed to create ticket: {{message}}', { message }));
    }
  };

  const saving = createMutation.isPending;
  const authorDisplay = user?.name ?? '—';
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div>
          <div className="text-sm text-gray-500">
            ...
            <Link to={adminRoutes.tickets} className="hover:underline">
              {t('createTicket.breadcrumb.tickets', 'Support Tickets')}
            </Link>
            {' › '}
            <span className="text-gray-700">{t('createTicket.breadcrumb.create', 'Create Ticket')}</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-bold text-[#0d0e0e] mt-1">
            {t('createTicket.title', 'Create Support Ticket')}
          </h1>
        </div>
        <div className="flex gap-2">
          ...
          <Button
            ...
            disabled={saving || optionsQuery.isLoading}
            className="bg-[#0d0e0e] text-white hover:bg-[#0d0e0e]/90"
          >
            <Save className="h-4 w-4 mr-1" />
            {saving ? t('common.saving', 'Saving…') : t('createTicket.save', 'Save Ticket')}
          </Button>
        </div>
      </div>
            ...
            <div className="space-y-5">
              ...
              <div>
                <Label className="text-sm font-medium">
                  {t('createTicket.fields.description', 'Description')} <span className="text-red-500">*</span>
                </Label>
            ...
            <div className="space-y-4">
              ...
              <div className="border-t border-gray-100 pt-4 space-y-2 text-xs">
                <div className="flex items-center justify-between text-gray-500">
                  ...
                  <span className="text-gray-700 font-medium truncate max-w-[160px]">{authorDisplay}</span>
                </div>
                <div className="flex items-center justify-between text-gray-500">
                  <span>{t('createTicket.sidebar.created', 'Created')}</span>
                  <span className="text-gray-700">{t('createTicket.sidebar.notSavedYet', 'Not saved yet')}</span>
                </div>
              </div>
            </div>
