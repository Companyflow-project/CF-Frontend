      ...
      <Table className="w-full table-fixed text-[13px]">
        ...
        <TableBody className="[&_tr]:last:border-b">
          {stats.length === 0 ? (
            ...
            stats.map((stat) => (
              <TableRow key={stat.employeeId} className="border-b border-[#ebf3ef] hover:bg-[#f6fbf9]">
                <TableCell className="pl-6 py-4">
                  <Checkbox
                    checked={selectedIds.includes(stat.employeeId)}
                    onChange={() => onSelect?.(stat.employeeId)}
                    className="rounded-[4px] border-[#3d997d] h-4 w-4"
                  />
                </TableCell>
